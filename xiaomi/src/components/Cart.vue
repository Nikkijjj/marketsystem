<template>
	<div class="content">
			...
			<tbody>
				<tr v-for="(item, index) in cart" :key="index">
					<td class="col01 one-line" :title="item.text">
						...
						&nbsp;&nbsp;
						<img class="shangpimg-goods" :src="getImg(item.cover)">&nbsp;
						{{ item.text }}
					</td>
					<td class="col02">￥{{ item.price }}</td>
					<td class="col03">
						<input class="ipt-quantity" type="number" v-model="item.quantity" @change="toggleQuantity"
							min=1>
					</td>
		...
		<div class="placeholder" v-else>亲!您还未添加任何东西，先去商城看看去吧~~</div>
		<div class="banner-pay">
			<span>￥&nbsp;{{ getTotalPrice() }}</span>&nbsp;&nbsp;&nbsp;&nbsp;<span class="btn-pay"
				@click="payAll">结&nbsp;&nbsp;算</span>
		</div>
	</div>
</template>
