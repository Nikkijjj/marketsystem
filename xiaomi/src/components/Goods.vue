<template>
	<div class="container">
		<div class="left">
			<div class="actor">
				<img :src="getImg(actorC)">
				<div ref="animPart" class="anim-part" :class="{'anim-cart': isCart, 'anim-order': isOrder}">
					<img :src="getImg(actorC)">
				</div>
			</div>
			<ul class="shangpimg-list">
				<li class="shangpimg-item" v-for="(shangpimg, index) in shangpimg" :key="index"
					@mouseover="toggleActor(index)">
					<img class="shangpimg" :src="getImg(shangpimg)">
				</li>
			</ul>
		</div>
		<div class="right">
			...
			<div class="banner-quantity">
				<input class="ipt-quantity" v-model="quantity" type="number" min=1 @keyup="checkQuantity">
			</div>
			<div class="banner-operate">
				<span class="btn-operate btn-buy" @click="addInOrder">
					立即购买
				</span>
			...
			<div class="banner-state">
				<p class="params-item">
					...
					<span class="params-label deep-gray">小米服务</span>
				</p>
			</div>
		</div>
	</div>
